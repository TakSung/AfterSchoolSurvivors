...
sys.path.append(r'C:\Users\Rgorithm2\Documents\jungwoosung\AfterSchoolSurvivors\src')
...
from components.enums import EntityStatus, ItemType, EnemyType
...
class VisualIntegrationTest:
    ...
    def create_sprite(self, shape: str, color: tuple[int, int, int], size: int | tuple[int, int]) -> pygame.Surface:
    ...
    def reset_game(self):
        ...
        self.entity_manager.add_component(player_id, PositionComponent(x=SCREEN_WIDTH / 2, y=SCREEN_HEIGHT / 2))
        self.entity_manager.add_component(player_id, HealthComponent(current=100, maximum=100, status=EntityStatus.ALIVE))
        self.entity_manager.add_component(player_id, VelocityComponent(dx=0, dy=0))
        
        player_sprite_surface = self.create_sprite('circle', COLOR_PLAYER, PLAYER_SIZE)
        player_rect = player_sprite_surface.get_rect(center=(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2))
        self.entity_manager.add_component(player_id, SpriteComponent(surface=player_sprite_surface, rect=player_rect))
    ...
    def create_enemy(self, position: tuple[int, int] | None = None):
        ...
        self.entity_manager.add_component(enemy_id, EnemyComponent(enemy_type=EnemyType.KOREAN_TEACHER))
        self.entity_manager.add_component(enemy_id, PositionComponent(x=x, y=y))
        self.entity_manager.add_component(enemy_id, HealthComponent(current=50, maximum=50, status=EntityStatus.ALIVE))

        # Velocity towards player
        if self.player_entity:
            player_pos = self.entity_manager.get_component(self.player_entity.id, PositionComponent)
            if player_pos:
                dx = player_pos.x - x
                dy = player_pos.y - y
                ...
                self.entity_manager.add_component(enemy_id, VelocityComponent(dx=vel_x, dy=vel_y))

        enemy_sprite_surface = self.create_sprite('rect', COLOR_ENEMY, ENEMY_SIZE)
        enemy_rect = enemy_sprite_surface.get_rect(center=(x, y))
        self.entity_manager.add_component(enemy_id, SpriteComponent(surface=enemy_sprite_surface, rect=enemy_rect))
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                player_vel = self.entity_manager.get_component(self.player_entity.id, VelocityComponent) if self.player_entity else None
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_k:
                        ...
                        for entity in self.entity_manager.get_entities_with_components(EnemyComponent):
                            self.entity_manager.destroy_entity(entity.id)
                    if event.key == pygame.K_x:
                        if self.player_entity:
                            player_comp = self.entity_manager.get_component(self.player_entity.id, PlayerComponent)
                            if player_comp:
                                player_comp.experience += 10
                                print(f"--- Added 10 EXP. Total EXP: {player_comp.experience} ---")
